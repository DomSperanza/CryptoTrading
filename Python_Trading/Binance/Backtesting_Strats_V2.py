...
def applyindicators(df,strat):
    ...
    if strat == 'Boom':
        ...
        df['HMA_200'] = ta.trend.wma_indicator(2*ta.trend.wma_indicator(df.Close,window = round(400/2))-ta.trend.wma_indicator(df.Close,window = 400),window = round(math.sqrt(400)))
        df['Hull_color'] = np.where(df.HMA_200>df.HMA_200.shift(2),'green','red')
        ...
        HP2 = [0]*len(df)
        LPP2 = 27
        alpha1222 = (math.cos(.707 * 2 * math.pi / 100) + math.sin(.707 * 2 * math.pi / 100) - 1) / math.cos(.707 * 2 * math.pi / 100)
        K22 = .3
        ...
        for i in range(2,len(df)):
            number = (1-alpha1222/2)**2*(df.iloc[i].Close-2*df.iloc[i-1].Close+df.iloc[i-2].Close) + 2*(1-alpha1222)*HP2[i-1] - (1-alpha1222)**2 *HP2[i-2]
            HP2[i] = number
    ...
    if strat == 'Magic':
        ...
        #ADX and DI
        length = 14
        TR= ta.volatility.average_true_range(df.High,df.Low,df.Close,window = 1)
        DMP = np.where((df.High-df.High.shift(1))>(df.Low.shift(1)-df.Low),df.High-df.High.shift(1),0)
        DMP = np.where(DMP<0,0,DMP)
        DMM = np.where((df.Low.shift(1)-df.Low)>(df.High-df.High.shift(1)),df.Low.shift(1)-df.Low,0)
        ...
        DXtest = pd.Series(abs(DIPlus-DIMinus)/(DIPlus+DIMinus)*100,index = df.index)
        ...
        for i in range(1,len(df)):
            MGma[i] = MGma[i-1]+(df.iloc[i].Close-MGma[i-1])/(length*(df.iloc[i].Close/MGma[i-1])**4)
        MGma = pd.Series(MGma,index = df.index)
        TR= pd.Series(ta.volatility.average_true_range(df.High,df.Low,df.Close,window = 1))
        ...
        df['BarColor'] = np.where(df.Close>upperk,'Blue',np.where(df.Close<lowerk,'Red','Gray'))
...
def GetTradesdf(buydates,buyprice,selldates,sellprice):
    ...
    tradesdf['profit_bool_fee'] = np.where(tradesdf.profit_net>0,True,False)
    return tradesdf
...
def Plot_visual(df,tradesdf,symbol = 'Coin Title'):
    ...
    plt.title(symbol)
    plt.plot(df[['SMA_50']])
    plt.scatter(tradesdf.buydates, tradesdf.buyprices, marker = '^',color = 'g', s = 200)
    plt.scatter(tradesdf.selldates, tradesdf.sellprices, marker = 'v',color = 'r', s = 200)
    plt.grid()
    plt.show()
