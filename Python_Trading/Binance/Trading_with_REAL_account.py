...
func.Plot_all_symbols_chan(indicators_dict,symbols)
i=0
...
while True:
    ...
    try:
        tradesdf, tsl_dicts, indicators_dict = func.trader(20,client,tradesdf,tsl_dicts,strategy, order )
        account_df = client.get_account()
        account_df = pd.DataFrame(account_df['balances'])
        account_df = account_df[account_df['asset'].isin(['BTC','ETH','BNB','SOL','USDT','USD'])]
        for symbol in symbols:
            print(client.get_symbol_ticker(symbol = symbol))
            if len(tsl_dicts[symbol])>0:
                print(tsl_dicts[symbol][['lastPrice','Benchmark','TSL']].tail(5))
